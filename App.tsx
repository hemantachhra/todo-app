...
const App: React.FC = () => {
    ...
    try {
      ...
      const sessionPromise = connectTaskEntryAPI({
        onopen: () => {
          ...
          // Proactive start
          sessionPromise.then(s => s.sendRealtimeInput({ text: "Ria, initiate briefing protocol." }));
        },
        onmessage: async (message: any) => {
          ...
          if (message.serverContent?.turnComplete) {
            const fullSentence = transcriptBuffer.trim();
            setAssistTranscript(prev => (prev + " " + fullSentence).trim());
            setTranscriptBuffer('');
            // AUTO-SLEEP Logic
            if (fullSentence.toLowerCase().includes("terminated") || fullSentence.toLowerCase().includes("good luck")) {
              setTimeout(() => { setShowTaskForm(false); stopVoiceSession(); }, 3500);
            }
          }
          if (message.toolCall) {
            for (const fc of message.toolCall.functionCalls) {
              if (fc.name === 'update_task_field') {
                ...
                if (field === 'category') setFormCategory(value.includes('5x') ? '5x_speed' : 'routine');
                if (field === 'priority') {
                  const v = value.charAt(0).toUpperCase() + value.slice(1).toLowerCase();
                  if (v === 'Important' || v === 'Priority') setFormUrgency(TaskUrgency.IMPORTANT);
                  else setFormUrgency(v as TaskUrgency);
                }
                if (field === 'alarm') setAlarmEnabled(value.toLowerCase() === 'on');
                ...
                setAssistTranscript("MISSION SECURED. READY FOR NEXT INPUT.");
  ...
  return (
    <div className="min-h-screen pb-24 bg-[#020617] text-slate-100 overflow-x-hidden">
      <header className="px-6 pt-10 pb-8 flex justify-between items-end sticky top-0 z-40 bg-[#020617]/95 backdrop-blur-xl border-b border-white/5">
        <div><h1 className="text-3xl font-black italic text-white uppercase tracking-tighter">TO DO LIST</h1><p className="text-[10px] text-blue-500 font-black uppercase tracking-[0.4em]">Ria Strategic Link</p></div>
        <button onClick={() => { setEditingTask(null); setShowTaskForm(true); }} className="w-14 h-14 rounded-2xl bg-blue-600 flex items-center justify-center text-3xl shadow-xl active:scale-90 transition-all">+</button>
      </header>

      <main className="px-6 pt-6 max-w-2xl mx-auto">
        {activeTab === 'routine' && (
          <div className="space-y-8">
            <button onClick={() => { setShowTaskForm(true); setTimeout(startAiAssistSession, 500); }} className="w-full py-6 rounded-3xl bg-blue-600/10 border-2 border-blue-500/30 flex items-center justify-center gap-4 text-blue-400 font-black uppercase tracking-widest hover:bg-blue-600 hover:text-white transition-all">🎙️ Initiate Ria Link</button>
        ...
        <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-2xl flex items-start justify-center p-4 pt-safe overflow-y-auto">
          <div className="w-full max-w-lg bg-slate-900 rounded-[2.5rem] p-8 border border-blue-500/30 shadow-2xl mt-32 mb-10 relative">
            <div className="absolute -top-16 left-1/2 -translate-x-1/2">
              <button onClick={isAiAssistActive ? stopVoiceSession : startAiAssistSession} className={`w-32 h-32 rounded-full border-4 flex flex-col items-center justify-center transition-all ${isAiAssistActive ? 'bg-amber-600 border-amber-400 animate-pulse shadow-[0_0_30px_rgba(217,119,6,0.5)]' : 'bg-blue-600 border-blue-400'}`}>
                <span className="text-4xl">{isAiAssistActive ? '📡' : '🎙️'}</span>
                <span className="text-[10px] font-black uppercase">{isAiAssistActive ? riaStatus : 'ASSISTANT'}</span>
              </button>
            </div>
            <div className="mt-16 mb-8 text-center">
              <h2 className="text-2xl font-black italic uppercase text-blue-400">{editingTask ? 'Modify Mission' : 'New Mission'}</h2>
              <div className="p-4 bg-blue-500/10 border border-blue-400/30 rounded-2xl mt-4 min-h-[60px] text-sm font-bold text-white italic">
                {assistTranscript || transcriptBuffer || 'Ria standby...'}
              </div>
            </div>
            <form onSubmit={e => { e.preventDefault(); saveCurrentForm(); }} className="space-y-6">
              <textarea value={formObjective} onChange={e => setFormObjective(e.target.value)} readOnly={isAiAssistActive} placeholder="Objective..." className="w-full bg-slate-950 border border-slate-800 rounded-2xl p-4 text-white outline-none" rows={1} />
              ...
              <div className="space-y-5">
                 <div className="flex justify-between items-center mb-1">
                   <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Progress: {initialProgressValue}%</label>
                   <button type="button" onClick={() => setIsFormProgressUnlocked(!isFormProgressUnlocked)} className={`text-[10px] font-black px-4 py-2 rounded-xl border ${isFormProgressUnlocked ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-500'}`}>{isFormProgressUnlocked ? '🔓 UNLOCKED' : '🔒 LOCKED'}</button>
                 </div>
                 <input type="range" min="0" max="100" value={initialProgressValue} disabled={!isFormProgressUnlocked} onChange={e => setInitialProgressValue(parseInt(e.target.value))} onPointerUp={() => setIsFormProgressUnlocked(false)} className="w-full h-4 bg-slate-800 rounded-lg appearance-none accent-blue-500" />
                 <div className="grid grid-cols-2 gap-2">
                    <button type="button" onClick={() => setFormCategory('routine')} className={`py-3 text-xs font-black uppercase border rounded-xl ${formCategory === 'routine' ? 'bg-blue-600 border-blue-400 text-white' : 'bg-slate-950 border-slate-800 text-slate-500'}`}>Routine</button>
                    <button type="button" onClick={() => setFormCategory('5x_speed')} className={`py-3 text-xs font-black uppercase border rounded-xl ${formCategory === '5x_speed' ? 'bg-amber-600 border-amber-400 text-white' : 'bg-slate-950 border-slate-800 text-slate-500'}`}>5x Speed</button>
                 </div>
                 <div className="grid grid-cols-3 gap-2">
                    {Object.values(TaskUrgency).map(urg => (
                      <button key={urg} type="button" onClick={() => setFormUrgency(urg)} className={`py-3 text-[10px] font-black uppercase border rounded-xl ${formUrgency === urg ? 'bg-blue-600 border-blue-400 text-white' : 'bg-slate-950 border-slate-800 text-slate-500'}`}>{urg}</button>
                    ))}
                 </div>
              </div>
              <div className="pt-8 flex flex-col gap-3">
                <button type="submit" className="w-full py-5 bg-blue-600 text-white font-black rounded-2xl uppercase shadow-xl shadow-blue-600/30">Launch Mission</button>
                <button type="button" onClick={() => { setShowTaskForm(false); stopVoiceSession(); }} className="w-full py-2 text-slate-600 text-[10px] font-black uppercase">Abort protocol</button>
              </div>
            </form>
          </div>
        </div>
